...
jQuery.fn.ingrid = function(o){
	...
	var cfg = {
		...
		dataType: 'xml',						// 'html' or 'json' - expected dataType returned
	...
	if (cfg.paging) {
		...
		if (cfg.totalRecords > 0) {
			...
			pv.updateViewInfo(totr, cfg.page);
		}
		...
		// start page button
		var pb1		= jQuery('<a href="'+cfg.currentview+'&page=1'+'">&laquo;</a>').addClass(cfg.pageStartClass).click(function(){
									p.setPage(1);
								});
	
		// prev page button
		if (cfg.page >1){
		var prevp=cfg.page - 1;
		}else{
			var prevp=1
		}
		var pb2		= jQuery('<a href="'+cfg.currentview+'&page='+ prevp +'">&lt;</a>').addClass(cfg.pagePrevClass).click(function(){
									var _p = p.getPage();																															
									if (_p > 1) {
										_p--;
										p.setPage(_p);
		...
		if (cfg.maxpage > cfg.page) {
		var nextp=cfg.page + 1 ;
		}else{
			var nextp=cfg.page
		}
		var pb3		= jQuery('<a href="'+cfg.currentview+'&page='+ nextp +'">&gt;</a>').addClass(cfg.pageNextClass).click(function(){
									var _p = p.getPage(); _p++;
									if (totp) {
		...
		// page field & form
		var pfld  = jQuery('<input type="text" value="' + cfg.page + '"/>').addClass(cfg.pageInputClass);
