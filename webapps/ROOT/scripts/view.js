		...
		success: function(xml){	
			...
			$(xml).find('users').find('entry').each(function(){
				$(".viewtable").append("<tr class='entrylist'>" +
						"<td width='2%' style='border:1px solid #ccc'><center><input type='checkbox' name='chbox' value='"+$(this).find("userid").text()+"'/></center></td>" +
...
function resetShedule(){	
	$.ajax({
		type: "POST",
		url: "Provider?type=service&operation=reload_schedule",
		data: "",
		success: function(xml){
			$(xml).find('response').each(function(){
				var st=$(this).attr('status');
				if (st=="error"){
					alert("the shedules have been not reloaded" );
				}else{
					alert("the shedules have been reloaded");					
				}	
			} );
		}
	});
...
function delUser(){	
	...
	for (i = 0; i < chBoxes.length; i++) {	
		if (chBoxes[i].checked){		
			...
			$.ajax({
				type: "POST",
				url: "Provider?type=delete&element=user&id=" + userID,
				data: "",
				success: function(xml){
					$(xml).find('response').each(function(){
						var st=$(this).attr('status');
						if (st=="error"){
							...
							if ($(this).find("signal").text() == "RELOAD_PAGE"){ 
								window.location.reload();
							}
						}	
					} );
	...
	for (i = 0; i < chBoxes.length; i++) {	
		if (chBoxes[i].checked){		
			...
			$.ajax({
				type: "POST",
				url: "Provider",
				data: "type=structure&id=delete_organization&dbid=" + dbID + "&key=" + orgID,
				success: function(xml){
					$(xml).find('response').each(function(){
						var st=$(this).attr('status');
						if (st=="error"){
